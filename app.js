...
(() => {
  ...
  generateBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      // Only students with at least one absence belong on a "Hostel
      // Absentees" report — anyone fully present is left off the list.
      const absentees = buildAbsenteeRows(students, attendance, dates, rate);

      if (absentees.length === 0) {
        setStatus(
          "Nobody was absent in this date range — nothing to report.",
          "is-error"
        );
        return;
      }
      ...
      renderPreview(absentees, dayHeaders, rate);

      const baseName = buildOutputBaseName(startDate, endDate);
      await downloadWorkbook(absentees, dayHeaders, baseName + ".xlsx");
      downloadPdf(absentees, dayHeaders, baseName + ".pdf");
  ...
  // Reduces the full roster down to students with at least one absence,
  // and computes their marks/fine once so the preview, .xlsx and .pdf
  // all draw from exactly the same rows. Order follows first appearance
  // in the source file, same as the original script.
  function buildAbsenteeRows(students, attendance, dates, rate) {
    const rows = [];
    for (const id of Object.keys(students)) {
      ...
      if (missed > 0) {
        rows.push({ name: s.name, hostel: s.hostel, marks, missed, fine: missed * rate });
      }
    }
    return rows;
  }
  ...
  function renderPreview(absentees, dayHeaders, rate) {
    ...
    for (const row of absentees) {
      const cells = row.marks.map((m) => (m ? `<td class="absent">x</td>` : `<td></td>`)).join("");
      totalMissedDays += row.missed;
      totalFine += row.fine;
      tbody += `<tr><td>${escapeHtml(row.name)}</td><td>${escapeHtml(row.hostel)}</td>${cells}<td class="total">${row.fine.toLocaleString("en-NG")}</td></tr>`;
    }
    tbody += "</tbody>";
    ...
    summaryEl.innerHTML = [
      ["Absentees listed", absentees.length],
  ...
  async function downloadWorkbook(absentees, dayHeaders, outputName) {
    ...
    for (const row of absentees) {
      ws.addRow([row.name, row.hostel, ...row.marks, row.fine]);
    }
  ...
  function downloadPdf(absentees, dayHeaders, outputName) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: "letter" });

    const head = [["Name/Date", "Hostel", ...dayHeaders, "Total Fine"]];
    const body = absentees.map((row) => [
      row.name,
      row.hostel,
      ...row.marks,
      row.fine.toLocaleString("en-NG"),
    ]);
